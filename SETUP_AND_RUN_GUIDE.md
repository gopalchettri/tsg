...
- **`APP_ENV`** — `prod` by default. **Fail-closed:** in `staging`/`prod` the app **refuses to start**
  if authentication is bypassed **or** the asset↔entity binding is `none`.
- **Asset↔entity binding** — `ASSET_ENTITY_BINDING=service` (default) resolves ownership via
  `asset.tier1_critical_service_id → onboarding_service_entity.service_id → group_id`. Nothing extra to
  set for your schema; adjust `ASSET_SERVICE_COLUMN` only if the asset→service column differs.

> **✅ You can run `APP_ENV=prod`** — the service-based ownership binding is implemented, so the
> fail-closed guard is satisfied by your real schema (no dev bypass needed).
...
**beat** (fires the reaper every 60s), and **redis**.
...
- *(Prometheus metrics + OpenTelemetry traces are Milestone 4 — see §12.)*
...
- **Observability & scale proof** — Prometheus metrics, OpenTelemetry traces, Flower, and a load test
  to size GPU/worker capacity; full OpenShift manifests (M4).
