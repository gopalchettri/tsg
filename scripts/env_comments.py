...
COMMENTS: dict[str, str] = {
    ...
    "TSG_LOCAL_MODEL_THREADPOOL_SIZE": """
        How many local model jobs (embedding or reranking) one worker process runs at the same
        time. Together they use half the CPUs that process may use (its container limit
        included); the other half is left for everything else. 1, the default, means one job at a
        time with that whole share: as fast overall as 2, and each job finishes twice as soon.
        They run on their own threads, separate from the ones network connections use.
    """,
