...
COMMENTS: dict[str, str] = {
    ...
    "TSG_LOCAL_MODEL_THREADPOOL_SIZE": """
        How many local model jobs (embedding or reranking) one worker process may run at the same
        time. Each job gets an equal share of the CPUs that process may use (its container limit
        included), so its jobs never exceed them: 2 on a 12-core box means 2 jobs using 6 cores
        each. Each worker process takes its own share. Raise it for more jobs in parallel, each
        one slower. They run on their own threads, separate from the ones network connections use.
    """,
