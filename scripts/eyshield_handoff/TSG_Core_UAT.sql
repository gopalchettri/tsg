...
--
-- *** HARD CUTOVER, 2026-08-30. *** This run RENAMES Threat_Scenario_Output to
-- Threat_Scenario, together with its PK, CHECK, DEFAULT and three indexes. An old and a
-- new application build CANNOT both run against one database: the old build refuses to
-- boot once the rename lands, and the new build cannot start until it has. There is no
-- safe ordering and no rolling deploy. STOP the API and the Celery workers, run this
-- script, run 6. TSG_Verify.sql, deploy the matching code, then start.
--
-- On a UAT database that already has the TSG tables, this run will:
--   * rename Threat_Scenario_Output -> Threat_Scenario plus PK_Threat_Scenario_Output,
--     CK_ScenarioOutput_DecisionExclusive, DF_ScenarioOutput_ScenarioNumber,
--     IX_ScenarioOutput_SessionSubActive, IX_ScenarioAudit_Output and
--     UX_TreatmentPlan_ActiveOutput (see the cutover warning above)
