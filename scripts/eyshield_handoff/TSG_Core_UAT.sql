...
-- current schema. The body below is `1. TSG_Core.sql` verbatim: every statement
-- is guarded against the live catalog, so whatever this database is missing gets
-- applied and everything already in place no-ops. Safe to re-run; never drops a
-- table or a column.
--
-- *** HARD CUTOVER *** This run RENAMES Threat_Scenario_Output to Threat_Scenario,
-- together with its PK, CHECK, DEFAULT and three indexes. An old and a new
-- application build CANNOT both run against one database, and there is no rolling
-- deploy. STOP the API and the Celery workers, run this script, run
-- 6. TSG_Verify.sql, deploy the matching code, then start.
--
-- On a database that already has the TSG tables, this run also:
--   * widens Risk_Treatment_Plan Status / TreatmentStrategy / RiskLevel /
--     ReviewStatus to nvarchar(100), and ErrorReason to nvarchar(max)
--   * creates any table or column added since the last run (guarded CREATEs/ADDs)
--   * creates IX_TreatmentPlan_SessionHistory, and DROPs the retired
--     IX_Session_CompletedByAsset if present
--   * fixes up data: backfills Risk_Treatment_Plan.ErrorReason on legacy ERROR
--     rows, and renames the review verdict 'changes_requested' -> 'rejected'
--   * on OLDER databases: widens Scenario_Session.StageStatus and
--     Subsystem_Stage_State.Status, rebuilds UX_Scenario_ActiveIdentity to include
--     ScenarioNumber, drops the legacy IX_SubsystemStageState_SessionSubLevel, and
--     relaxes CrmRiskIdentificationID to nullable
--
-- SECTION 0 (RCSI) no-ops when RCSI is already ON. If it is OFF, enabling it
-- forces every other session off the database — run in a quiet window.
--
-- RUN: paste the whole file into SSMS (F5) and read the Messages pane, or
--   sqlcmd -b -I -S <server> -d <database> -E -i TSG_Core_UAT.sql
-- (-I sets QUOTED_IDENTIFIER ON, required by the filtered indexes.)
-- AFTERWARDS: run 6. TSG_Verify.sql.
--
-- KEEP IN LOCKSTEP with 1. TSG_Core.sql: on the next schema change, regenerate
-- this file as this header plus that file's body, verbatim.
-- ============================================================================
