...
On accept, threats whose grounding score is **below the promotion threshold** feed the library — but asymmetrically, following the prompt contract (§9.1):

- The threat **type** (generic, library-shaped by construction: the prompt forbids asset/product names in it) is auto-promoted into `Threat_Type` with source `ai_auto_promoted`, plus actor links when the actor list was validated.
- The threat **name** is *never* auto-minted into `Threat_Catalogue`, because the prompt *requires* names to embed the asset's own name ("*Unauthorized disclosure of Citizen Personal Information*"), which is never library idiom. It goes to `Threat_Candidate_Review` as `pending` for a curator.
...
This asymmetry is why accept can safely auto-promote types but never names (§6.8). Other contract points: `category` must be one of the live `Threat_Category` names; `actors` is a closed list when the `Threat_Actor` table is seeded (grounding drops out-of-set actors by exact string match, so inviting free-form labels would waste proposals); language must be defensive and risk-framed (no exploit instructions); and the model is told plainly that *"you decide nothing"* — every proposal is independently checked. In additive rounds a coverage-exclusion list is included and *"an empty array is a valid answer"*.
...
| `treatment_prompt` | treatment plan | 1 per plan | JSON plan: objective, recommendation, justification, recommended controls, remediation action plan (role owners, relative timelines), mitigation activities, residual-risk assessment, assumptions — stored in `PlanJSON` (row semantics §10.3) |
...
4. **Admin key, double-gated**: routers require the admin key (constant-time compare on *bytes* — the string overload raises on non-ASCII, which would have turned a failed auth into a 500) *and* a per-route principal, so shared-key actions are still attributed to a real user in the audit trail. An unconfigured key always denies.
...
| D6 | **Identity-fold dedup, not similarity** | Dedup must be deterministic and index-enforceable; the three-rung key uses the finest *verified* library id, and semantic similarity is demoted to an advisory sibling warning. |
...
| D9 | **Type promotion only; names to curator review** | Enforced by the prompt contract's name/type asymmetry: types are library-shaped by construction, names never are. Auto-minting names would pollute the catalogue with asset-specific entries. |
