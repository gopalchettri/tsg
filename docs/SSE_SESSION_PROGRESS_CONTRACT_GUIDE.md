...
`app/pipeline/cascade.py`, `app/pipeline/reaper.py`, `app/sse/bus.py`) — this file is the wire
...
/v1/sessions/{session_id}` (the *board*) is designed to agree with the stream at all times — poll
...
headers on every request — `X-API-Key`, `X-User-Id`, `X-Entity-Id` (see
`docs/TSG_API_AUTHENTICATION_GUIDE.md`) — and `EventSource` has no way to set custom headers.
...
|---|---|---|---|
| `reconcile` | session (full snapshot) | — it *is* the durable snapshot | `SessionBoard` (+ injected `type`) |
...
| `treatment_plan_result` | plan (no subsystem) | **NONE on this board** — poll `GET /v1/sessions/{id}/treatment-plans` | `TreatmentPlanResultEvent` |
| `heartbeat` | none (keep-alive) | n/a — carries no state | `HeartbeatEvent` |

All ten are registered in `/openapi.json` on `GET /v1/sessions/{session_id}/events` as one
Pydantic union (`app/api/sessions.py::_EVENT_STREAM_RESPONSES`) — generate a typed client from the
spec instead of hand-copying the shapes below.

**One subsystem on this stream.** `subsystem_id` is always `0` (`ASSET_UNIT_ID`) on every event
...
`SessionBoard` — the same body `GET /v1/sessions/{session_id}` returns, plus one injected key:
...
(`app/api/sessions.py`) — it is **not** part of the `SessionBoard` model itself (that model is
...
**It is a snapshot, not a delta**, and it does **not** carry treatment-plan state — fetch
...
```json
{"type": "stage_started", "session_id": "3fa85f64-…", "subsystem_id": 0,
 "stage": "THREATS", "status": "RUNNING", "generation_epoch": 1,
 "ts": "2026-08-12T09:00:00+00:00"}
```

`stage`: `"THREATS"` or `"SCENARIOS"`. `status` is always `"RUNNING"` — this event fires only at
claim time.

### 2.3 `stage_completed`

```json
{"type": "stage_completed", "session_id": "3fa85f64-…", "subsystem_id": 0,
 "stage": "SCENARIOS", "status": "SCENARIOS_AWAITING_DECISION", "generation_epoch": 1,
 "ts": "2026-08-12T09:02:14+00:00"}
```

**`status` has exactly two real values, not one** — both published from the same call site
(`tasks.py::_send_live_update`): THREATS finishes `"COMPLETE"`; SCENARIOS finishes
`"SCENARIOS_AWAITING_DECISION"` (`StageStatus.AWAITING_DECISION`'s wire value) — reaching
AWAITING_DECISION for SCENARIOS *is* the review barrier, not an unfinished state. A failed stage
never emits this event — it routes to `error` instead.

### 2.4 `subsystem_started`

```json
{"type": "subsystem_started", "session_id": "3fa85f64-…", "subsystem_id": 0,
 "generation_epoch": 1, "ts": "2026-08-12T08:59:59+00:00"}
```

Fires **before** this subsystem's stages run. No `stage` field — it precedes both THREATS and
...
```json
{"type": "session_entered_review", "session_id": "3fa85f64-…",
 "status": "SCENARIOS_AWAITING_DECISION", "generation_epoch": 1,
 "ts": "2026-08-12T09:02:15+00:00"}
```

Fires once, when every subsystem has hit its review barrier. Session-wide — never carries
`subsystem_id`. **This is one of the two signals a client may use to tear down a "generating…"
UI state** (the other being a terminal `session_status` from a poll/reconcile) — see §2.6.

### 2.6 `error` — read this one carefully

```json
// scope="stage" — tasks.py::_record_failure
{"type": "error", "session_id": "3fa85f64-…", "scope": "stage", "subsystem_id": 0,
 "message": "the AI service is temporarily unavailable — please retry",
 "generation_epoch": 1, "ts": "2026-08-12T09:01:40+00:00"}

// scope="session" — tasks.py::_mark_session_failed
{"type": "error", "session_id": "3fa85f64-…", "scope": "session",
...
- **`scope="stage"`**: one subsystem's stage errored. `subsystem_id`/`generation_epoch` are
  present. The session is **not necessarily done** — other subsystems can still finish, or the
...
- **`scope="session"`**: every subsystem errored; the session was marked `cancelled`
  server-side. `subsystem_id`/`generation_epoch` are absent — terminal.
- **`scope` is absent (`null`)** on the **three** `error` events `app/pipeline/reaper.py`'s
  ...
  — a known gap, not closed by this contract; see §4.1. Fall back to "`subsystem_id` present" for
  those specifically.
...
field — the board's own `progress.threats`/`progress.scenarios` status (`ERROR`) plus
`progress.error_message[stage]` (item 7 — see §5) already carry the same information durably. A
missed `error` event costs nothing beyond the client not being told the instant it happened.

### 2.7 `next_set_result` (unchanged by this phase — documented for completeness)

```json
{"type": "next_set_result", "session_id": "3fa85f64-…", "subsystem_id": 0,
...
reason→sentence table). **Durable mirror: `progress.last_next_set`** — compare `.epoch` against
the `epoch` your `POST .../scenarios/next-set` call returned.

### 2.8 `regen_result` (unchanged by this phase — documented for completeness)

```json
{"type": "regen_result", "session_id": "3fa85f64-…", "subsystem_id": 0,
 "requested_scenario_ids": ["6ba7b810-…"], "new_scenario_ids": ["b3fc2c96-…"],
 "replacements": [{"old": "6ba7b810-…", "new": "b3fc2c96-…"}],
 "failed_threat_ids": [], "rescored_threat_ids": [], "reason": null, "detail": null,
 "message": null, "ts": "2026-08-12T09:06:30+00:00"}
```

**Durable mirror: `progress.last_regen`** (new this phase — see §5). Compare `.epoch` against the
...
`requested_scenario_ids`/`new_scenario_ids`) — `last_regen` is built straight from the
`regeneration_completed` audit row, not from this event, so the two are not byte-identical.

### 2.9 `treatment_plan_result` (unchanged by this phase — documented for completeness)

```json
{"type": "treatment_plan_result", "session_id": "3fa85f64-…",
 "scenario_id": "1a2b3c4d-…", "plan_id": "b9fe2c07-…", "status": "COMPLETE",
 "reason": null, "ts": "2026-08-12T09:14:02+00:00"}
```

**Match on `scenario_id`, never `plan_id`** — a regeneration mints a new `plan_id` for the same
scenario. **No durable mirror on this board at all** — see §4.3, and
`docs/TREATMENT_PLAN_API_TEST_GUIDE.md` §3.2 for the full contract.

### 2.10 `heartbeat`

```json
{"type": "heartbeat", "session_id": "3fa85f64-…", "ts": "2026-08-12T09:00:15+00:00"}
```

Sent every `sse_ping_seconds` (default 15s) when nothing else was published, so proxies don't
drop an idle connection. Carries no state — use its arrival only to reset your own idle timer.
...
|---|---|---|
| A worker is SIGKILLed mid-stage | **Nothing**, for up to `stage_lease_seconds` (default 300s, or auto-derived from `llm_timeout_seconds`/`llm_max_retries`) + `reaper_interval_seconds` (default 60s) — the board shows a frozen `RUNNING` the whole time, then the reaper's sweep publishes `error` once it reclaims the lease | Detection is lease-based by design (no heartbeat-per-stage exists); see §4.2 |
| A treatment plan's worker dies mid-generation | Never — the `Risk_Treatment_Plan` row stays `RUNNING` forever; there is no reaper for it | `docs/TREATMENT_PLAN_API_TEST_GUIDE.md` §3.2's "Never publishes" table |
| A treatment plan hits an LLM-capacity autoretry | Never — the retry keeps bumping the row's progress clock, so it never even goes stale | ibid |
| A treatment plan is cancelled or reviewed | Never — cancel/review run in the API process; only the worker publishes | ibid |
| Any `bus.publish` call fails (Redis down/slow) | **Every** event from that worker process is silenced for `sse_breaker_cooldown_seconds` (default 30s) — not just the one that failed | Process-global circuit breaker (`app/sse/bus.py`); see §4.4 |
| A client connects between the `reconcile` snapshot and the live subscription actually starting | An event published in that narrow window is dropped — never delivered, never replayed | Known, accepted race; see §4.5 |
| `verify_membership` revokes a user mid-stream | No event fires for it — the stream simply **closes** (server-initiated) on the next tick | Not a publish gap — this is the re-authorization check (item 30), working as designed |
...
`reaper.py`'s three `error` sites (the dead-worker sweep, a different phase — item 5) do not yet.

**Recovery:** treat `scope == null` as "derive it yourself" — `subsystem_id` present means
...
"checking…" instead of a silent frozen status once staleness is expected. **That field does not
exist on `SessionBoard` as of this guide** — it was never scheduled into an implementation phase.

**Recovery:** there is no live signal for this window. A client-side heuristic — no
`stage_started`/`stage_completed`-adjacent progress for longer than your own configured "this is
taking too long" threshold — is the only available mitigation today. Once the reaper's sweep runs
(`reaper_interval_seconds`, default 60s, after the lease itself expires), an `error` event (scope
absent — see §4.1) or a `reconcile`-visible status change follows.
...
does not — plan state lives entirely outside `SessionBoard`.
...
`scenario_id`. Full detail: `docs/TREATMENT_PLAN_API_TEST_GUIDE.md` §3.2.
...
diagnosability), not a client-visible signal — this is not something a client can detect on the
wire at all.

**Recovery:** this is exactly why every event in this guide has (or should have — see §4.3) a
...
**Recovery:** the same backstop poll as §4.4 covers this — the missed event's underlying state
...
There is deliberately no replay log — `reconcile` plus the durable mirrors above cover
...
`treatment_plan_result` (§4.3) and the reconcile-to-subscribe race window itself (§4.5), both
...
`decide_session_outcome`, never overrides it — including on the path a regenerate takes back into
...
its callers (several sites in `cascade.py`, plus `reaper.py` and `sessions.py`) — out of scope for
...
`progress.last_next_set.epoch` from `GET /v1/sessions/{session_id}` instead — both are written from
...
**This changes `GET /v1/sessions/{session_id}`'s response shape, not just the SSE stream** — both
...
| Two simultaneous stage failures | Last-row-wins — one message silently overwrote the other | Both survive, keyed by stage |

```json
// before
{"progress": {"error_message": "the AI service is temporarily unavailable — please retry"}}

// after
{"progress": {"error_message": {"threats": "the AI service is temporarily unavailable — please retry"}}}
```
...
carries an error message — check `progress.threats == "ERROR"` /
...
parse the response the moment this ships.** There is no dual-write/versioning transition — this
...
| `sse_breaker_cooldown_seconds` | 30 | How long a worker process silences ALL publishes after one failure (§4.4) |
...
| `stage_completed` | stage | no (SCENARIOS→AWAITING_DECISION *is* the review gate) | `progress.{threats,scenarios}` |
...
| `treatment_plan_result` | plan | no | **none — poll `/treatment-plans`** |
| `heartbeat` | none | no | n/a |

---

## 8. Migration: Phase 4 result-field renames (BREAKING, `/results` and friends)

**This stream is NOT affected.** Every event shape above is unchanged. The rename lands on the
*result* payloads — `GET /v1/sessions/{id}/results`, `/accepted-scenarios`, the cross-session
scenario reads, and the `.xlsx` exports — which now spell each field exactly as the database
column it comes from. Same precedent as the `error_message` str -> dict change documented in
section 6: a typed client reading the old names fails to parse rather than silently mis-reading.
...
* **The scenario body** (`scenario.*`: `scenario_title`, `scenario_statement`, `risk_statement`,
  `threat_category`, `supporting_system_applicability`) keeps snake_case. Those are the model's
  own JSON keys, passed through verbatim; they are not database columns.
* **The session envelope** (`session_id`, `entity_id`, `user_id`, `asset_id`, `asset_name`,
  `session_status`, `completed_at`, `progress`) keeps its wire spelling, so
  `AcceptedScenariosResponse` and the items inside it agree with each other.
* **Derived, non-column fields** keep snake_case: `moderation_checked`, `moderation_flagged`,
  `moderation_categories`, `validation_status`, `validation_errors`, `replaced_scenarios`.
* **Request bodies, query parameters and route paths are untouched.** This is a response-shape
  change only — nothing you send has to change.

### The one behaviour change worth reading twice

`LibraryThreatType` / `LibraryThreatName` are now reported **alongside** `ThreatType` /
`ThreatName` instead of silently replacing them. Before, the API returned
`LibraryThreatType or ThreatType` in a single field, so a caller could not tell the model's
wording from the curator's — and therefore could not judge match quality. Both are on the wire
now, and the only place the preference still applies is the **display prose** inside
`scenario.threat_type` / `scenario.threat_name`, which prefers the curated wording.
...
## 10. Migration: Phase 5 identifier rename (BREAKING, and THIS TIME THE STREAM MOVES)

Read this even if you skipped section 8. Phase 4 renamed the result fields snake_case -> PascalCase
so each one spelled its database column. Phase 5 **reverses that** and goes further: the wire now
names the ENTITY, not the table it lives in, and spells every field in snake_case. Section 8 is kept
as history — its "Now" column describes Phase 4 and is no longer the current contract.

**Section 8 promised "This stream is NOT affected." That promise does not carry over.** Three event
fields change here, and a typed client reading the old names fails to parse rather than silently
mis-reading:

| Event | Was | Now |
|---|---|---|
| `regen_result` | `requested_output_ids`, `new_output_ids` | `requested_scenario_ids`, `new_scenario_ids` |
| `treatment_plan_result` | `output_id` | `scenario_id` |

Everything else on the stream — event names, `reconcile`, `heartbeat`, `progress`, `coverage` — is
unchanged.

### The rename, in one rule

`OutputID` was vague: it named the table rather than the thing. The identifier is now `scenario_id`
on the wire and `ScenarioID` in the database, and the same rule renamed its siblings:

| Was | Now |
|---|---|
| `OutputID` / `output_id` / `{output_id}` (path) | `scenario_id` / `{scenario_id}` |
| `ThreatActorID` | `actor_id` |
| `ControlLibraryID` | `control_id` |
| `ThreatID`, `ThreatTypeID`, `ThreatCatalogueID`, `SubsystemID` | `threat_id`, `threat_type_id`, `threat_catalogue_id`, `subsystem_id` |
| `Accepted`, `Superseded`, `ScenarioNumber`, `CreatedAt`, `Score`, `MapRank` | `accepted`, `superseded`, `scenario_number`, `created_at`, `score`, `map_rank` |

Request bodies moved with them: `POST .../scenarios/reject` and the accept subset now take
`{"scenario_ids": [...]}`, not `{"output_ids": [...]}`. Path parameters are positional, so
`{output_id}` -> `{scenario_id}` costs a client nothing.

### The flat threat block is GONE — read the nested one

`AcceptedScenario` used to carry flat copies of the threat fields ALONGSIDE the nested `threat`
object built from the same row. The flat copies are deleted. Each scenario carries its own `threat`
object, so which threat produced which scenario is unambiguous per scenario — deliberately NOT
de-duplicated into a shared threat list keyed by id, which would save bytes and force every consumer
to do the join by hand.

`threat_actors` is no longer a bare list of names — it is a list of keyed refs carrying `actor_id`
beside the name, so a client can join without string-matching.

### Timestamps now say they are UTC

Every datetime on the wire is serialized with a trailing `Z`. The stored values were ALWAYS UTC;
they simply never said so, and a browser parsing an unlabelled timestamp read it as LOCAL time —
5.5 hours wrong for an IST client on `accepted_at`, `reviewed_at` and every audit timestamp. If you
were compensating for that skew client-side, remove the compensation.

### Additive in the same release (nothing breaks if you ignore these)

* **Attribution on scenario reads:** `accepted_by` / `accepted_at`, `rejected_by` / `rejected_at`,
  and on sessions and plans `cancelled_by` / `cancelled_at`, `reviewed_by` / `reviewed_at`. These
  record WHO decided. Previously only reject stored it, and no endpoint returned it at all.
* **`GET /v1/sessions/{session_id}/audit`** — the full step trail, oldest-first, with `limit` and
  `offset` required. Every row names its own subject via `subject_type` (`session` | `scenario` |
  `plan`) plus `scenario_id` / `plan_id`, so when the actor changes between rows it is visibly
  because the SUBJECT changed, not because the trail is inconsistent.
* `actor_user_id` is **null for pipeline steps** and `actor_type` says so outright. Worker rows are
  no longer stamped with the session owner's name — that back-fill is what previously made a
