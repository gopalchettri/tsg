...
**One subsystem today.** `subsystem_id` is always `0` (`ASSET_UNIT_ID`) — the pipeline tracks one
...
{"type": "regen_result", "session_id": "3fa85f64-…", "subsystem_id": 0,
 "requested_output_ids": ["6ba7b810-…"], "new_output_ids": ["b3fc2c96-…"],
...
`requested_output_ids`/`new_output_ids`) — `last_regen` is built straight from the
...
{"type": "treatment_plan_result", "session_id": "3fa85f64-…",
 "output_id": "1a2b3c4d-…", "plan_id": "b9fe2c07-…", "status": "COMPLETE",
 "reason": null, "ts": "2026-08-12T09:14:02+00:00"}
```

**Match on `output_id`, never `plan_id`** — a regeneration mints a new `plan_id` for the same
...
`output_id`. Full detail: `docs/TREATMENT_PLAN_API_TEST_GUIDE.md` §3.2.
