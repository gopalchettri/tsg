...
> **2026-09-20 note (F5):** three of this row's pointers are stale. There is no **admin UI** — the admin CRUD routes were deleted 2026-08 as unused complexity, which left `create/get/update/soft_delete_library_row` with ZERO callers and, unnoticed for a year, left every `promote-to-library` row stranded at `IsActive=0` with nothing able to approve it. `db/dal.py:1957-1967` now holds next-set scoping, not the CRUD block; the bulk importer is `app/intel/library_import.py`, not `pipeline/threat_library_import.py`. The CRUD helpers regained a caller on 2026-09-20 (`POST /v1/tsg/threat-intel/library/threats/approve`), and `scripts/test_pipeline_guards.py` now fails the build if that caller disappears again. F5's point survives the correction: that route calls `update_library_row` with no `If-Match`, so two curators working one queue is still last-write-wins — now narrower, since the only field it writes is `IsActive` and both curators want it True.
