...
| New: `POST /v1/sessions/{session_id}/scenarios/{scenario_id}/unaccept` | Takes one acceptance back; the scenario returns to undecided and the review queue reopens for it. No body. `409` with `not_accepted` if it was not accepted. Its plan is kept and parks exactly as it does on a replace. **No schema change** |
| A regenerate or next-set click is no longer lost when the session is busy | The worker now retries a busy subsystem for about five minutes and, if it still cannot start, hands the stage back and publishes `reason: "subsystem_busy"`. Clients should show that message and let the user re-click. Previously the click vanished: the session read `complete` while accept answered 404, and a lost next-set left the asset reserved until the reaper cancelled the session |
