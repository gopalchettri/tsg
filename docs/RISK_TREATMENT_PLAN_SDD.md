...
| 0.13 | 16 Aug 2026 | — | **Review verdict renamed: `changes_requested` → `rejected`** (the old name read like a distinct workflow state; it always meant rejection). `TreatmentReviewStatus` keeps two members (`approved`/`rejected`); the review body's `decision` and the register's `review_status` filter remain enum-typed, so the old literal is now a clean 422 — a deliberate wire break. Stored rows are backfilled by an idempotent UPDATE in `1. TSG_Core.sql` (order-safe against the code deploy — stored values read back as raw text — but run it WITH the deploy so `review_status=rejected` filters match pre-rename rows). Audit `DetailJSON` history keeps the old literal: records are never rewritten. `TSG_Verify.sql` column-width row updated (longest member is now `approved`, 8 chars). |
| 0.14 | 16 Aug 2026 | — | **`ReviewStatus` widened nvarchar(20) → nvarchar(100)** (user request — headroom so a future verdict value can never hit the silent truncation-stall failure mode the Verify script's column-width check documents). Three sites in lockstep: the CREATE TABLE, the guarded ADD for pre-column databases, and a NEW guarded `ALTER COLUMN` widen for databases already provisioned at 20, following the established StageStatus-widen conventions: schema-qualified guard, `CHARACTER_MAXIMUM_LENGTH BETWEEN 1 AND 99` (idempotent AND excludes nvarchar(max)'s -1, so a max column can never be narrowed), nullability restated, own GO batch; ReviewStatus is in no filtered-index predicate, so no Msg 5074 exposure. `models.py` mapped width matches. `TSG_Verify.sql` needs no change — its check is declared ≥ needed, and 100 ≥ 8. |
| 0.15 | 16 Aug 2026 | — | **Register detail parity with the poll GET.** `?include_plan=true` register rows now carry `scenario` (statements + threat identity), `treatment_strategy` and `risk_identification_date` alongside `plan` — rendered by `_plan_status_from_row`, the poll GET's own presenter, so a detailed register row mirrors `GET …/treatment-plan`'s response (minus `superseded`, plus `asset_name`/`scenario_title`). `dal.entity_plan_rows(include_plan=True)` widens to the `active_plan_row` column set + the Scoped_Threat/Identified_Threat outer joins; flag-off SQL and wire stay byte-identical apart from three additive always-null keys. Not redundant IO (unlike the versions history): every register row is a different scenario. |
| 0.16 | 16 Aug 2026 | — | **`Risk_Treatment_Plan.Status` widened nvarchar(20) → nvarchar(100)** (user request), same three-site lockstep and guarded-widen conventions as v0.14's ReviewStatus change — schema-qualified guard, `BETWEEN 1 AND 99`, **NOT NULL restated** (ALTER COLUMN resets nullability), own GO batch; Status is in no index key or filtered-index predicate. `models.py` Unicode(100) matches. `TSG_Verify.sql` width row unchanged (`COMPLETE`, 8 chars, still the longest stored value — `timed_out` is projection-only and never stored). |
| 0.17 | 16 Aug 2026 | — | **`TreatmentStrategy` (30→100, NOT NULL restated), `RiskLevel` (10→100) and `ErrorReason` (30→100) widened to nvarchar(100)** (user request), completing the uniform width policy on `Risk_Treatment_Plan`'s enum-vocabulary columns. Same guarded-widen conventions, one GO batch per column; the guarded ADD blocks now create RiskLevel/ErrorReason at 100; §4.1 table updated (incl. the previously missed Status row). `models.py` matches. `TSG_Verify.sql` width rows unchanged — longest stored values (`Mitigate` 8, `Critical` 8, `generation_failed` 17) all fit, and the check still guards future growth. |
| 0.18 | 16 Aug 2026 | — | **`ErrorReason` → nvarchar(max)** (user request, superseding v0.17's 100 for this column). CREATE, guarded ADD and the widen block all move to max; the widen guard fires on any bounded width (`CHARACTER_MAXIMUM_LENGTH <> -1`). Reason codes are short and stored in-row, so max is free at rest; the column is in no index (nvarchar(max) cannot be an index key). `models.py` uses `UnicodeText` (house style for max). The Verify width row stays: the width check auto-skips max columns and re-arms if the column is ever re-narrowed. |
...
| D5 | **Accepted scenarios only.** `Accepted != 1` → 409. (The former `Superseded = 1` gate is RETIRED: since scenario acceptance was decoupled from generation recency, the accepted version may itself be superseded — `TreatmentGateReason.scenario_superseded` remains in the enum for wire-compat but is never raised.) | User decision; risk treatment logically follows acceptance. |
| D6 | **Create/regenerate are SEPARATE routes (2026-08-18 revision); regeneration via supersede.** One active plan per scenario (`WHERE Superseded = 0`). `POST .../treatment-plan` is FIRST generation only (any existing plan → 409 `plan_already_exists`); `POST .../treatment-plan/regenerate` (body: optional `user_note` only — register data reused from the active version's frozen snapshot) supersedes and inserts. The review route's optional `plan_id` atomically switches an older COMPLETE version back in on approve (`treatment_plan_version_restored` audit event; rejecting history → 409 `version_not_active`). No epochs. | The index is the race arbiter (precedent: `UX_Session_ActiveAsset` + `create_session`'s IntegrityError→409). Epochs fence *reused* stage rows; plan rows are never reused. History preserved for free. Wire details: docs/TSG_UI_API_INTEGRATION.md. |
...
| TreatmentStrategy | nvarchar(100) | No | | — | `'Mitigate'` — **server-stamped** (`TreatmentStrategy.mitigate`), never a request field |
| Status | nvarchar(100) | No | | — | `StageStatus`: RUNNING / COMPLETE / ERROR |
...
This route is **first generation only** (2026-08-18 revision): if ANY plan row exists for the scenario → 409 `plan_already_exists`. Every later version comes from `POST .../treatment-plan/regenerate` (body: optional `user_note` only; register data reused from the active version's frozen snapshot), which carries the old re-POST semantics: on a `COMPLETE`/`ERROR` active plan → supersede + new row; fresh `RUNNING` → 409 `generation_in_progress`; **stale** `RUNNING` (`UpdatedAt` older than `treatment_stale_seconds`) → takeover. Baseline is always the ACTIVE row — never newest-by-date — so a version switched back in by an approve is what a later regenerate builds on.
...
| 409 | treatment_conflict | `details.reason ∈ {scenario_not_accepted, plan_already_exists, generation_in_progress, version_not_active, not_complete, not_in_progress}` (`TreatmentGateReason`) |
...
| `scenario_superseded` | HISTORICAL — never raised since scenario acceptance was decoupled from recency; kept for wire-compat |
| `plan_already_exists` | Create refused: a plan exists for this scenario — use `POST .../treatment-plan/regenerate` |
| `version_not_active` | Reject refused: the target is a history version, already not the plan — rejecting it is meaningless |
| `generation_in_progress` | A fresh RUNNING plan row exists for this scenario (deliberately shares its value with `ReviewGateReason.generation_in_progress` — same meaning, different route family); also blocks a version switch |
...
3. `scn = dal.scenario_row(sess, session_id, output_id)` — absent → `NotFoundError`; `Accepted != 1` → `TreatmentConflict(reason=scenario_not_accepted)`. (The former `Superseded == 1 → scenario_superseded` gate is RETIRED — the accepted version may itself be superseded. On the create route only, an existing active plan → `TreatmentConflict(reason=plan_already_exists)`; both routes share this flow via `_launch_generation`, regenerate passing its `plan_id`-fenced supersede target and snapshot-derived `risk_input`.)
...
4. **Manual E2E** (real header auth: `X-API-Key` + identity headers, docs/TSG_API_AUTHENTICATION_GUIDE.md — no seeding beyond TSG's own data; the body carries the risk data): create session → accept → POST treatment-plan with a full body (202) → duplicate POST (409 `plan_already_exists`) → rating out of range / bad `risk_level` (422) → non-accepted scenario (409) → poll GET to COMPLETE → verify the plan: `treatment_plan == "Mitigate"`, `controls_to_be_implemented` matches, the three echoes match the body, `risk_identification_date` echoed on the row → POST `/regenerate` with only `user_note` (202, new plan_id; old row `Superseded=1`; snapshot register block carried) → approve the OLD version via review `plan_id` (atomic switch: old current+approved, `version restored` audit event, one active row) → reject-history (409 `version_not_active`). SSMS: `Prompt_Log` rows with `Stage='treatment_plan'` whose prompt text contains **no `risk_owner` name**; audit events requested=`user`, outcome=`system`. Kill the worker mid-run → GET projects ERROR after the window → `/regenerate` takes over. (Executed 2026-08-18 against the local stack — all steps green.)
