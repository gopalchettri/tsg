...
(SDD Appendix B). Rule: application code, query builders, and serializers
...
class SessionStatus(StrEnum):
    ...
    active = "active"        # holds the M4 one-active-session-per-asset lock
...
class SessionMode(StrEnum):
    ...
    AUTO = "AUTO"   # the only mode implemented: all 3 stages run back-to-back per subsystem,
                    # stopping only at the single REVIEW gate. A future step-by-step MANUAL
                    # mode is reserved (SDD Appendix B) but has no code path today.
...
class ThreatRuleType(StrEnum):
    """`Config_Threat_Rule.RuleType` — the three scoping rule families (SDD §5.4, [R12]).
    ...
    relevance_context_value = "relevance_context_value"
...
class ValidationStatus(StrEnum):
    """Deterministic output-validation result (SDD §5.2/§5.6). These checks FLAG,
...
class AuditEventType(StrEnum):
    ...
    grounding_summary = "grounding_summary"             # written once per subsystem after Stage 2 (THREATS); DetailJSON carries the threat count
    scoping_complete = "scoping_complete"               # written once per subsystem after Stage 3 scoring; DetailJSON carries scoped/selected counts
