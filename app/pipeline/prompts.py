...
def treatment_prompt(snapshot: dict[str, Any]) -> list[dict]:
    ...
    system_content = (
        ...
        "the action ids.\n"
        "mitigation_timeline: one relative overall duration for the whole plan. EVERY timeline "
        "in this plan is measured from the plan's start (day 0), never from a "
        "dependency's completion — a chain of dependent actions must still finish, end to "
        "end, inside the overall duration. When risk_assessment.assessment_window is "
        "non-null, the ENTIRE plan must complete inside it: keep mitigation_timeline and "
        "every action timeline within assessment_window.total_days days - never schedule "
        "past timeline_end_date. If the window is too short for a defensible plan, still "
        "schedule inside it AND say so plainly in action_plan. Timeline fields are always "
        "relative durations — the window's calendar dates may be referenced in action_plan "
        "prose but never used as a timeline value.\n"
        ...
        "shortest defensible duration (inside the assessment window when one is supplied). "
